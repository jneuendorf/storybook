...
class Stories extends React.Component {
  ...
  mapStoriesHierarchy(storiesHierarchy) {
    ...
    if (treeModel.namespaces) {
      const childItem = {
        type: treeNodeTypes.BUTTON,
        label: 'test',
        parent: treeModel,
      };
      if (treeModel.namespaces.length === 0) {
        childItem.name = 'component';
      } else if (treeModel.namespaces.length === 1) {
        childItem.name = 'story';
      }
      treeModel.children.push(childItem);
    }
    return treeModel;
  }
